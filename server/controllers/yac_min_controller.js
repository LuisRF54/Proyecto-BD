...
yacMinCtrl.getYacMinerales = async (req, res) => {
    await client.query("SELECT fk_mi_id AS Mineral, fk_y_id AS Yacimiento FROM Yac_Min")
        .then(response => {
...
yacMinCtrl.getMinYac = async (req, res) => {
    ...
    await client.query("SELECT fk_mi_id AS Mineral, fk_y_id AS Yacimiento FROM Yac_Min WHERE fk_mi_id = "+id)
        .then(response => {
            if(response.rowCount)
                res.json(response.rows);
            else
...
yacMinCtrl.getYacMin = async (req, res) => {
    const id = req.params.id;
    await client.query("SELECT fk_mi_id AS Mineral, fk_y_id AS Yacimiento FROM Yac_Min WHERE fk_y_id = "+id)
        .then(response => {
            if(response.rowCount)
                res.json(response.rows);
            else
