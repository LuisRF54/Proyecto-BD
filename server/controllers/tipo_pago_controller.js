...
tipoPagoCtrl.createTransferencia = async (req,res) => {
    const tp = req.body;
    await client.query("INSERT INTO Tipo_Pago (tp_tipo,tp_banco,tp_fecha) VALUES ('Transferencia','"+tp.banco+"','"+tp.fecha+"')")
        .then(response => {
            res.json('Insertado');
        })
        .catch(err => {
...
tipoPagoCtrl.createTarCredito = async (req,res) => {
    const tp = req.body;
    await client.query("INSERT INTO Tipo_Pago (tp_tipo,tp_banco,tp_tipo_tar_cre,tp_fecha_vencimiento) VALUES ('Tar_credito','"+tp.banco+"','"+tp.tipo+"','"+tp.vencimiento+"')")
        .then(response => {
            res.json('Insertado');
        })
        .catch(err => {
...
tipoPagoCtrl.createTarDebito = async (req,res) => {
    const tp = req.body;
    await client.query("INSERT INTO Tipo_Pago (tp_tipo,tp_banco,tp_tipo_tar_deb) VALUES ('Tar_debito','"+tp.banco+"','"+tp.tipo+"')")
        .then(response => {
            res.json('Insertado');
        })
        .catch(err => {
