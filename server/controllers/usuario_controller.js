...
userCtrl.getUsuarios = async (req, res) => {
    await client.query("SELECT u_nombre AS login, u_password AS password, fk_em_id AS Empleado, fk_r_clave AS Rol FROM Usuario")
        .then(response => {
...
userCtrl.getUsuario = async (req, res) => {
    ...
    await client.query("SELECT u_nombre AS login, u_password AS password, fk_em_id AS Empleado, fk_r_clave AS Rol FROM Usuario WHERE u_clave = "+id)
        .then(response => {
            if(response.rowCount)
                res.json(response.rows);
            else
    ...
    await client.query("SELECT u_nombre AS login, u_password AS password, fk_em_id AS Empleado, fk_r_clave AS Rol FROM Usuario WHERE u_nombre = '"+user.login+"' AND u_password = '"+user.password+"'")
        .then(response => {
            if(response.rowCount)
                ...
                res.json('Usuario y/o contraseña incorrecto');
        })
        .catch(err => {
            console.log(err);
            res.json('Ha ocurrido un error');
